            ...
            struct stat *target_stat ATTRIBUTE_UNUSED)
{
  ...
  /* if input file failed to open, or failed to move file 
  offset to the beginning of input file */
  if (fromfd < 0 || lseek(fromfd, 0, SEEK_SET) != 0)
    return -1;

  /* open the destination file to write, and truncate it if file
  already exists.
  Windows distinct O_BINARY and O_TEXT */
  ...
  // copy file until complete
  while ((nread = read(fromfd, buf, sizeof buf)) > 0)
  {
    /* if failed to write the same # of bytes to destination
    as read from the input, exit */
    if (write(tofd, buf, nread) != nread)
    {
      /* make sure the errno records the failure regarding write()
          rather than close() */
      saved = errno;
      close(fromfd);
      close(tofd);
...
int smart_rename(const char *from, const char *to, int fromfd,
